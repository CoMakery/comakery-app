class TopContributors
  ...
  def top_contributors(project, n)
    all_columns = (Account.column_names - %w[password_digest public_address nonce network_id system_email new_award_notice]).map { |column| "max(accounts.#{column}) as #{column}" }.join(', ')
    Account
