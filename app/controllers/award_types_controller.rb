class AwardTypesController < ApplicationController
  ...
  def create
    ...
    if ImagePixelValidator.new(@award_type, award_type_params).valid? && @award_type.save
      set_ok_response
  ...
  def update
    if ImagePixelValidator.new(@award_type, award_type_params).valid? && @award_type.update(award_type_params)
      set_ok_response
      render json: @ok_response, status: :ok
    else
      set_error_response
