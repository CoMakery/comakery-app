module ECIES
  ...
  class Crypt
    ...
    CIPHERS = %w[AES-128-CBC AES-192-CBC AES-256-CBC AES-128-CTR AES-192-CTR AES-256-CTR].freeze
    ...
    def initialize(cipher: 'AES-256-CTR', digest: 'SHA256', mac_length: :half, kdf_digest: nil, mac_digest: nil, kdf_shared_info: '', mac_shared_info: '') # rubocop:todo Metrics/CyclomaticComplexity, Metrics/ParameterLists
      ...
      %i[half full].include?(mac_length) or raise 'mac_length must be :half or :full'
    ...
    def decrypt(key, encrypted_message)
      ...
      ciphertext_length.positive? or raise OpenSSL::PKey::ECError, 'Encrypted message too short'
    ...
    def kdf(shared_secret, length, shared_info_suffix)
      ...
      raise 'length too large' if length / @kdf_digest.digest_length >= 0xFF_FF_FF_FF
      ...
      loop do
        ...
        return io.string.byteslice(0, length) if io.pos >= length
      end
    end

    # @return [String] A string representing this Crypt's parameters.
    def to_s
      "KDF-#{@kdf_digest.name}_HMAC-SHA-#{@mac_digest.digest_length * 8}-#{@mac_length * 8}_#{@cipher.name}"
    end
    ...
    # Extracted from Money-Tree
    # https://github.com/GemHQ/money-tree/blob/2686577092d3d4e2af8de8552782ff633e61e652/lib/money-tree/key.rb#L61
    #
    # @param [OpenSSL::PKey::EC] The private key.
    # @return [String] Hex representing public key.
    def self.calculate_public_key(key)
      key.private_key? or raise 'Must be a private key'

      key.group.generator.mul(key.private_key).to_octet_string(:uncompressed).unpack1('H*')
    end
  end
end
