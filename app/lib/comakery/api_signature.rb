...
# }
#
# nonce_history = ["ajgpe79rv6sv1i8sqhxobd", "ajgpe79rv6sv1i8sqhxobd", "ajgpe79rv6sv1i8sqhxobd"]
# http_url = "https://example.org/"
# http_method = "GET"
#
# begin
#   Comakery::APISignature.new(request, http_url, http_method, nonce_history).verify(public_key)
...
module Comakery
  ...
  class APISignature
    ...
    # @param nonce_history [Array<String>] history of nonces for processed requests during defined timestamp window (optional)
    def initialize(request, http_url = '', http_method = '', nonce_history = [])
      @request = request
      @nonce_history = nonce_history
      @http_url = http_url
    ...
    def sign(private_key)
      ...
      @request['proof']['signature'] = Base64.strict_encode64(
        signing_key.sign(@request['body'].to_json)
      )
    ...
    private
      ...
      def verify_http_url
        raise Comakery::APISignatureError, 'Invalid URL' unless @request['body']['url'] == @http_url
      end

      def verify_http_method
        raise Comakery::APISignatureError, 'Invalid HTTP method' unless @request['body']['method'] == @http_method
      end

      def verify_nonce
        raise Comakery::APISignatureError, 'Invalid nonce' if @nonce_history.include?(@request['body']['nonce'])
      end

      def verify_timestamp
        raise Comakery::APISignatureError, 'Invalid timestamp' unless @request['body']['timestamp'].between?(timestamp - TIMESTAMP_EXPIRATION_SECONDS, timestamp + TIMESTAMP_AHEAD_SECONDS)
      end

      def verify_type
        raise Comakery::APISignatureError, 'Invalid proof type' unless @request['proof']['type'] == PROOF_TYPE
      end

      def verify_method(public_key)
        raise Comakery::APISignatureError, 'Invalid proof verificationMethod' unless @request['proof']['verificationMethod'] == public_key
      end
