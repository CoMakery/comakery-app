class Views::Accounts::Show < Views::Base
  ...
  def content
    text react_component(
      'Account',
      {
        currentAccount: current_account.as_json(only: %i[email first_name last_name nickname date_of_birth country qtum_wallet ethereum_auth_address ethereum_wallet cardano_wallet bitcoin_wallet eos_wallet tezos_wallet constellation_wallet occupation linkedin_url github_url dribble_url behance_url]).merge(
