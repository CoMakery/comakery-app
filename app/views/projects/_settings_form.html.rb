    ...
    class SettingsForm < Views::Base
            ...
            row {
                ...
                with_errors(project, :license_finalized) {
                  label {
                    ...
                    div(class: 'help-text') do
                      text 'Leave this unchecked if you want to use #{Rails.application.config.project_name} for tracking contributions with no legal agreement for sharing revenue.'
                    end
                  }
                }
              }
              ...
              column('large-6 small-12') {
                ...
                div(class: "project-token-terms #{'hide' if project.revenue_share?}") {
                  ...
                  p {
                    link_to 'Send us an email', "mailto:#{Rails.application.config.company_email}"
          ...
          full_row {
            column {
              with_errors(project, :public) {
                label {
                  f.check_box :public
                  text " Set project as publicly visible on #{Rails.application.config.project_name} "
                  question_tooltip "Decide whether or not to display this project in the #{Rails.application.config.project_name} project index"
                }
              }
      ...
      def ethereum_beta(form)
        if current_account.slack_auth.slack_team_ethereum_enabled?
          ...
          label {
            link_to 'Contact us', "mailto:#{Rails.application.config.company_email}"
