...
describe GetAwardableAuthentications do
  ...
  describe '#call' do
    it 'returns some auths' do
      ...
      result = described_class.call(current_account: project_owner, project: project)
      expected = [
        ['@blah', '47'],
        ['Bob - @bob', 'U2229UVMH'],
    ...
    it "doesn't include the current_account if they are NOT the owner of the project" do
      ...
      result = described_class.call(current_account: other_auth.account, project: project)
      expect(result.awardable_authentications).to eq([])
    end

    context 'without a current user' do
      it 'returns an empty array' do
        result = described_class.call(current_account: nil, project: nil, authentications: [])
        expect(result.awardable_authentications).to eq([])
      end
    end
  end
