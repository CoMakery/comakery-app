  ...
  describe "#call" do
    ...
    describe "with all award.quantity = 1" do
      ...
      it "defaults to top 5 contributors ordered by
        ...
        expect(TopContributors.call(projects: [sb_project]).
            contributors[sb_project].map { |auth| auth.slack_user_name }).
            to eq(%w(sb3 sb1 sb2 sb5 sb4))
      end

      it "can return a specified number of top contributors" do
        expect(TopContributors.call(projects: [sb_project], n: 3).
            contributors[sb_project].
            map { |auth| auth.slack_user_name }).
            to eq(%w(sb3 sb1 sb2))
      end
      ...
      it %{ can return a specified number of top contributors} do
        expect(TopContributors.call(projects: [sb_project], n: 3).
            ...
            to eq([["sb3", 2000, 4.days.ago], ["sb1", 2000, 5.days.ago], ["sb2", 1000, 1.days.ago]])
      end
    end

    describe "with some award.quantity > 1" do
      before do
        create(:award, authentication: sb_auth2, award_type: large_award_type, quantity: 1.5, created_at: 1.days.ago)
      end
      ...
      it %{returns the most awarded contributors, ordered by total contribution/recency,
        ...
        expect(TopContributors.call(projects: [sb_project], n: 3).
            contributors[sb_project].
            map { |auth| [auth.slack_user_name, auth.total_awarded.to_i, auth.last_awarded_at] }).
            to eq([["sb3", 2000, 4.days.ago], ["sb1", 2000, 5.days.ago], ["sb2", 1500, 1.days.ago]])
      end
    end
  end
end
