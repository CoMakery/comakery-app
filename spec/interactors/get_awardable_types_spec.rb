  ...
  describe "#call" do
    ...
    describe "can_award" do
      ...
      it "returns true if you are an owner of the project and your slack_auth slack_team_id is the same as the project's" do
        expect(GetAwardableTypes.call(project: citizencode_project, current_account: owner).can_award).to be_truthy
      end
      ...
      it "returns true if account's slack auth is the same as the project and the project has community awardable awards" do
        ...
        expect(GetAwardableTypes.call(project: swarmbot_project, current_account: swarmbot_only_account).can_award).to be_truthy
      end

      it "returns false otherwise" do
        expect(AwardType.where(community_awardable: true)).to be_empty

        expect(GetAwardableTypes.call(project: nil, current_account: owner).can_award).to be_falsey

        expect(GetAwardableTypes.call(project: citizencode_project, current_account: nil).can_award).to be_falsey
        expect(GetAwardableTypes.call(project: citizencode_project, current_account: swarmbot_only_account).can_award).to be_falsey
        expect(GetAwardableTypes.call(project: swarmbot_project, current_account: owner).can_award).to be_falsey
        expect(GetAwardableTypes.call(project: swarmbot_project, current_account: swarmbot_only_account).can_award).to be_falsey

        expect(GetAwardableTypes.call(project: other_project, current_account: owner).can_award).to be_falsey
        expect(GetAwardableTypes.call(project: other_project, current_account: swarmbot_only_account).can_award).to be_falsey
      end
    end
  end
end