...
describe Project do
  describe 'validations' do
    ...
    it 'rails error if not found Ethereum address' do
      ...
      expect { described_class.create(payment_type: 'project_token', ethereum_contract_address: '111', ethereum_network: 'N/A') }.to raise_error(ArgumentError)
    end
    ...
    describe '#ethereum_contract_address' do
      ...
      it 'validates with a valid ethereum address' do
        ...
        expect(build(:project, ethereum_contract_address: "0x#{'a' * 40}", ethereum_network: 'N/A')).to be_valid
        expect(build(:project, ethereum_contract_address: "0x#{'A' * 40}", ethereum_network: 'N/A')).to be_valid
      end
      ...
      it 'does not validate with an invalid ethereum address' do
        ...
        expect(build(:project, ethereum_contract_address: 'foo', ethereum_network: 'N/A').tap(&:valid?).errors.full_messages).to eq([expected_error_message])
        expect(build(:project, ethereum_contract_address: '0x', ethereum_network: 'N/A').tap(&:valid?).errors.full_messages).to eq([expected_error_message])
        expect(build(:project, ethereum_contract_address: "0x#{'a' * 39}", ethereum_network: 'N/A').tap(&:valid?).errors.full_messages).to eq([expected_error_message])
        expect(build(:project, ethereum_contract_address: "0x#{'a' * 41}", ethereum_network: 'N/A').tap(&:valid?).errors.full_messages).to eq([expected_error_message])
        expect(build(:project, ethereum_contract_address: "0x#{'g' * 40}", ethereum_network: 'N/A').tap(&:valid?).errors.full_messages).to eq([expected_error_message])
      end
  ...
  describe '#transitioned_to_ethereum_enabled?' do
    ...
    it 'is false if an existing project with an account is transitioned from ethereum_enabled = false to true' do
      project = create(:project, ethereum_enabled: false, ethereum_contract_address: '0x' + '7' * 40, ethereum_network: 'N/A')
      project.update!(ethereum_enabled: true)
      expect(project.transitioned_to_ethereum_enabled?).to eq(false)
    end
  end
  ...
  it 'can manual input token_symbol' do
    ...
    project = create :project, token_symbol: 'AAA', ethereum_contract_address: contract_address, ethereum_network: 'N/A'
