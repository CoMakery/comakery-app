...
describe Payment do
  specify do
    validation_errors = described_class.new.tap(&:valid?).errors.full_messages
  ...
  it 'reconciled false by default' do
    expect(described_class.new.reconciled).to eq(false)
  end
  ...
  describe 'validations' do
    ...
    it 'Payee can redeem shares that they do have' do
      award_type.awards.create_with_quantity(1, account: account)
    ...
    it 'can update the quantity_redeemed to an amount they have' do
      award_type.awards.create_with_quantity(2, account: account)
  ...
  describe 'scopes' do
    ...
    specify do
      expect(described_class.total_value_redeemed).to eq(21.00)
    end
  end
end
