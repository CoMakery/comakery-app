...
RSpec.describe OreIdService, type: :model, vcr: true do
  ...
  describe '#create_token' do
    ...
    context 'with recovery token' do
      specify do
        VCR.use_cassette('ore_id_service/token_for_recovery', match_requests_on: %i[method uri]) do
  ...
  describe '#reset_url' do
    before do
      expect(subject).to receive(:create_token).with('dummmyrecovery').and_return('test')
      expect(subject).to receive(:remote).and_return({ 'email' => 'dummyemail' })
    end
