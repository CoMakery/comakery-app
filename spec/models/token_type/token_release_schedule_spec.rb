...
describe TokenType::TokenReleaseSchedule do
  it_behaves_like 'a token type'

  specify { expect(described_class.new.name).to eq('TOKENRELEASESCHEDULE') }
  specify { expect(described_class.new.symbol).to eq('TOKENRELEASESCHEDULE') }
