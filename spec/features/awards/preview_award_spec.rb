...
describe 'preview award', js: true do
  ...
  context 'on tezos network' do
    let!(:project) do
      create(:project, title: 'Project that needs awards', account: account, maximum_tokens: 10000000, maximum_royalties_per_month: 1000000, token: create(:token, ethereum_enabled: true, contract_address: '2' * 40, symbol: 'BIG', decimal_places: 8, blockchain_network: 'tezos_mainnet', coin_type: 'xtz'))
    end
