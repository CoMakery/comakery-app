...
describe "viewing projects, creating and editing", :js, :vcr do
  ...
  let!(:owner_account) { create(:account, name: "Hubert").tap{|a| create(:authentication, account_id: a.id, slack_team_id: "team id")} }
  let!(:other_account) { create(:account, name: "Sherman").tap{|a| create(:authentication, account_id: a.id, slack_team_id: "team id")} }
