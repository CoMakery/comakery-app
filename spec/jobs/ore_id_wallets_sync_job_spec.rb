...
RSpec.describe OreIdWalletsSyncJob, type: :job do
  ...
  let(:self_reschedule_job) { double('OreIdWalletsSyncJob', perform_later: nil) }
  let(:ore_id) { create(:ore_id, skip_jobs: true) }

  before do
    allow(described_class).to receive(:set).with(any_args).and_return(self_reschedule_job)
    allow_any_instance_of(ore_id.class).to receive(:sync_remote!)
    Timecop.freeze(now)
  end
  ...
  context 'when sync is allowed' do
    ...
    it 'calls create_remote and sets synchronisation status to ok' do
      expect_any_instance_of(ore_id.class).to receive(:sync_remote!)
    ...
    context 'and service raises an error' do
      before { allow_any_instance_of(ore_id.class).to receive(:sync_remote!).and_raise }
