  ...
  describe "#show? #edit? #update?" do
    it "only allows viewing of projects that are public or are owned by the current account" do
      [:edit?, :update?].each do |action|
        ...
        expect(ProjectPolicy.new(account, others_private_project).send(action)).to be true

        expect(ProjectPolicy.new(other_team_member, others_public_project).send(action)).to be true
        expect(ProjectPolicy.new(other_team_member, my_private_project).send(action)).to be true
      end
