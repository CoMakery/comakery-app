    ...
    context "when the user goes straight to the beta sign up page so there isn't a beta signup" do
      ...
      context 'when user opts out' do
        it 'tells the user' do
          ...
          expect(response.status).to eq(302)
          expect(flash[:notice]).to eq("You have not been added to the beta waiting list. Check back to see new public #{Rails.application.config.project_name} projects!")
        end
      end
    end
  end
